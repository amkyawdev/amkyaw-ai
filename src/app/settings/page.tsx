...
import { 
  ...
  Zap, Brain, Trash2, Download, Upload, Check, X, Loader2, Cpu
} from 'lucide-react';
...
const Toggle = ({ enabled, onChange }: { enabled: boolean; onChange: () => void }) => (
  <button
    onClick={onChange}
    className={cn('w-14 h-8 rounded-full transition-all relative', enabled ? 'bg-orange-500' : 'bg-muted')}
  >
        ...
        <div className="space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} 
            ...
            <div className="flex gap-4">
              <button onClick={() => setTheme('dark')} 
                className={cn('flex-1 p-4 rounded-xl border', theme === 'dark' ? 'border-orange-500 bg-orange-500/10' : 'border-border hover:border-orange-500/50')}>
                <Moon className="w-6 h-6 mx-auto mb-2" />
                <span className="text-sm font-medium">Dark</span>
              </button>
              <button onClick={() => setTheme('light')} 
                className={cn('flex-1 p-4 rounded-xl border', theme === 'light' ? 'border-orange-500 bg-orange-500/10' : 'border-border hover:border-orange-500/50')}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} 
            ...
            <div className="flex gap-4">
              {(['small', 'medium', 'large'] as const).map((size) => (
                <button key={size} onClick={() => updatePreferences({ fontSize: size })} 
                  className={cn('flex-1 p-4 rounded-xl border', preferences.fontSize === size ? 'border-orange-500 bg-orange-500/10' : 'border-border hover:border-orange-500/50')}>
            ...
            <div className="grid grid-cols-3 gap-4">
              {Object.entries(GROQ_MODELS).map(([key, model]) => (
                <button key={key} onClick={() => setDefaultModel(key as GroqModelType)} 
                  className={cn('p-4 rounded-xl border', defaultModel === key ? 'border-orange-500 bg-orange-500/10' : 'border-border hover:border-orange-500/50')}>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} 
            ...
            <div className="space-y-4">
              ...
              <button onClick={handleExportChats} 
                className="w-full flex items-center justify-center gap-2 p-4 rounded-xl border border-border hover:border-orange-500/50">
                <Download className="w-5 h-5" />Export All Chats
              </button>
              <input type="file" ref={fileInputRef} accept=".json" className="hidden" />
              <button onClick={() => fileInputRef.current?.click()} 
                className="w-full flex items-center justify-center gap-2 p-4 rounded-xl border border-border hover:border-orange-500/50">
                <Upload className="w-5 h-5" />Import Chats
              </button>
              {showClearConfirm ? (
                <div className="flex items-center gap-2 p-4 rounded-xl bg-destructive/10 border border-destructive/30">
                  ...
                  <button onClick={() => setShowClearConfirm(false)} className="p-2 rounded-lg bg-muted">
                    <X className="w-4 h-4" />
                  </button>
                </div>
              ) : (
                <button onClick={() => setShowClearConfirm(true)} 
                  className="w-full flex items-center justify-center gap-2 p-4 rounded-xl border border-destructive/30 text-destructive hover:bg-destructive/10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} 
            ...
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">{isAuthenticated ? `Signed in as ${user?.email}` : 'Not signed in'}</p>
              <a href="/login" className="text-orange-500 hover:underline text-sm">{isAuthenticated ? 'Switch Account' : 'Sign In'}</a>
            </div>
          </motion.div>
        </div>
      </div>
